...
- Updated license to **OCL v1.2**: Oxiverse API is a paid subscription service (free tier for personal/research) — using it commercially requires NO separate OCL Commercial License and creates no derivative work (Section 2A(a)); self-hosted commercial use of **the Oxiverse Software (all current and future Oxiverse services, including RAVANA and IntentForge)** is permitted either under a **paid closed-source Commercial License** (Section 2A(b-i), the commercial product Oxiverse sells) or under the **free full-source path** where the entire stack is released under OCL or an OSI-approved open source license with no open-washing (Sections 2A(b-ii) & 3A); added explicit "free written confirmation of compliance" (Section 2A(d-i)) and the Open-Washing Prohibition (Section 3A). Clarifications (non-version-changing, still OCL v1.2): OCL is source-available (not OSI open source); "ENTIRE stack" excludes independent third-party infra (databases, proxies, hosting, external APIs); contributed code is never placed into a closed-source deployment; later versions are self-contained and do not retroactively alter earlier adopted versions.
